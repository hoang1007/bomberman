    ...
    GameObject.addObject(
        new GameObject() {
          ...
          public void start() {
            ...
            // d = 2*r + 1
            this.dimension =
                new Vector2D(attackRadius, attackRadius).add(this.dimension).multiply(2);
          }
          ...
          public void onCollisionStay(Collider that) {
            if (that.gameObject instanceof Bomber) {
              boolean isInside =
                  this.collider.getOverlapArea(that)
                      == that.gameObject.dimension.x * that.gameObject.dimension.y;

              if (isInside && chasePath.isEmpty()) {
                speed = 5.0;
                var rect =
                    Geometry.unionRect(
                        that.gameObject.position.clone().smooth(Sprite.DEFAULT_SIZE, 1),
                ...
                chasePath =
                    AStar.findPath(map, Oneal.this.position, that.gameObject.position, speed);
              }
            }
          }
